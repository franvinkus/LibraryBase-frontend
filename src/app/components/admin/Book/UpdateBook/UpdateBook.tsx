...
export default function UpdateBook({ isOpen, onClose, bookId }: UpdateBookProps) {
  ...
  const handleCategoryChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    const selectedCategory = categories.find((cat) => cat.cateId === selectedValue);
    if (selectedCategory && !selectedCategories.some((cat) => cat.id === selectedValue)) {
      setSelectedCategories([...selectedCategories, { id: selectedCategory.cateId, name: selectedCategory.cateName }]);
    }
  };
        ...
        <div className="mt-3 flex flex-wrap gap-2">
          {selectedCategories.map((category) => (
            <div key={category.id} className="bg-blue-500 text-white px-3 py-1 rounded-lg flex items-center">
              {category.name}{" "}
              <button onClick={() => removeCategory(category.id)} className="ml-2 text-sm">
                ✖
              </button>
            </div>
          ))}
